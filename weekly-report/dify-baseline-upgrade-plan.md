...
本表只描述公司在原 1.14 基线上已经完成的能力，不在这里给出迁移取舍。
...
迁移取舍按能力方向统一判断，不把接口适配、Adapter 调整等实现变化当成功能舍弃。

| 能力方向 | 升级建议 | 原因 |
| --- | --- | --- |
| 账号与工作空间 | **保留，做必要适配** | SkyOA、超级管理员、默认工作空间和权限治理都是公司业务规则，新版只能提供底层账号和空间能力，不能直接替代这些规则；整体仍有明确保留价值 |
| 托管执行引擎 | **保留核心能力，适配新版执行入口** | Policy、Admission、优先级、双 Worker、容量控制和 Job 管理仍是公司统一调度的核心价值；主要变化集中在 Worker 如何调用新版执行器、如何停止任务和交付结果，不需要因此舍弃整套引擎 |
| 应用执行 | **正式应用执行继续保留托管调度** | Workflow、Chatflow、Chat、Completion 和正式 Schedule 都属于正式业务执行链路，继续使用统一排队和容量治理价值较高；旧 AGENT_CHAT 可继续保留，新版独立 Agent 暂时按官方执行方式运行 |
| Console 调试与 Human Input | **托管调试和公司托管暂停恢复本期暂缓** | 草稿、单节点、Iteration、Loop 调试依赖新版内部调试入口和前后端状态协议；Human Input 的公司暂停数据格式与新版恢复上下文差异较大。这两部分迁移成本明显高于正式执行，本期可直接使用 1.17.1 官方调试和人工输入能力，不影响基础功能使用 |
| 存储与数据安全 | **保留必要公司接入和存量兼容** | KMS 仍决定公司 S3 的认证方式，历史文件路径和租户私钥直接关系到已有数据是否可读取；这些能力不能因升级而取消。通用上传、下载、预签名等能力则优先复用新版 |
| 管理与可观测 | **保留与托管能力直接相关的管理、健康、监控和审计；执行数据自动清理舍弃** | 任务管理、策略管理、Worker / Scheduler 健康和执行审计都依赖托管引擎，保留后仍需对应管理能力；通用 Trace 可复用新版 Unified Tracing；执行数据自动清理在原基线后续已被撤销，本次不恢复 |
| 部署与运行 | **保留，按最终迁移范围收敛运行角色** | 现有 DevOps 构建和运行方式仍可复用；正式托管执行继续保留后，Scheduler 和 Standard / Critical Worker 仍需要独立运行；本期暂缓的调试与暂停恢复不再增加额外部署链路 |
