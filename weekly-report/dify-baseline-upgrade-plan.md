...
以下只保留从 1.14 基线升级到 1.17.1 后新增的主要产品能力，不包含接口重构、权限细化、Bug Fix 和原有功能增强。

| 功能方向 | 新增功能 | 功能说明 | 本次处理 |
| --- | --- | --- | --- |
| Workflow | 自然语言生成 Workflow / Chatflow | 可以直接描述希望实现的流程，由 Dify 自动生成 Workflow 或 Chatflow，并在生成后继续调整 | 直接采用新版能力 |
| Agent | 新版 Agent | 新增独立的 Agent 应用形态，可单独创建、配置、运行和发布 | 采用新版能力，暂不接入公司托管调度 |
| Agent | Agent Skills | 可以在工作空间内创建和管理可复用 Skills，并供 Agent 调用 | 直接采用新版能力 |
| Agent | Agent 独立运行环境 | Agent 可以在独立运行环境中执行任务，并保存构建后的运行环境 | 按需启用 |
| Workflow | LLM Environment | 可以统一维护模型和参数配置，并由多个 Workflow 节点复用 | 直接采用新版能力 |
| CLI | difyctl | 可以通过命令行查看和运行 Dify 应用与 Workflow | 可选使用 |
| 可观测 | Unified Tracing | 提供统一的应用、Workflow、节点和 Agent 执行链路追踪 | 采用新版能力 |
| 密钥管理 | 外部 KMS Provider | 提供标准化外部密钥服务接入能力，可统一管理和轮换密钥 | 复用新版能力承接公司 KMS |

### 2.2 原基线开发功能升级取舍

| 能力方向 | 原基线已开发功能 | 本次升级保留内容 | 不迁移 / 舍弃内容 |
| --- | --- | --- | --- |
| 账号与工作空间 | SkyOA 登录、超级管理员、邀请注册、默认工作空间、工作空间管理与权限 | 公司账号准入、默认工作空间和管理规则继续保留 | 不保留旧版登录、账号和会话的内部实现，改用新版账号体系承接 |
| 托管执行引擎 | Policy、Admission、Queue、Scheduler、Worker、容量控制、任务管理、结果交付 | 统一排队、优先级、双 Worker、容量治理、任务管理继续保留 | 不再用公司旧实现替代 Dify 原生应用执行能力 |
| 应用执行与调试 | Workflow、Chatflow、Chat、Completion、Agent，以及草稿、单节点、迭代、循环、Human Input、定时触发接入托管调度 | 现有应用和调试场景继续接入公司托管调度 | 不迁旧版应用执行 Adapter；新版独立 Agent 暂不接入旧托管调度 |
| 存储与数据安全 | 公司 KMS、S3、文件路径、租户私钥、Redis 事件通信 | KMS、历史文件兼容、租户私钥和必要的事件通信继续保留 | 不迁整套旧 S3 实现，不重复建设新版已经提供的上传、下载、预览和预签名能力 |
| 管理与可观测 | 任务管理、Worker/Scheduler 健康、OTel、执行审计 | 调度专属管理、Worker/Scheduler 状态和执行审计继续保留 | 不重复建设新版已经具备的通用 Health、Trace 和基础运行监控 |
| 部署与运行 | API、Worker、Scheduler、环境配置和启动脚本 | 现有 DevOps 发布方式和独立运行角色继续保留 | 舍弃与新版运行方式重复或已经失效的旧启动、初始化和兼容逻辑 |
