...
| 能力方向 | 原基线已开发功能 | 本次升级处理 |
| --- | --- | --- |
| 账号与工作空间 | SkyOA 登录、超级管理员、邀请注册、默认工作空间、工作空间管理与权限 | 保留 |
| 托管执行引擎 | 统一准入、执行策略、优先级队列、Scheduler、Standard/Critical Worker、容量控制、任务管理、结果交付 | 保留 |
| 应用执行与调试 | Workflow、Chatflow、Chat、Completion、AGENT_CHAT，以及草稿、单节点、迭代、循环、Human Input、定时触发接入托管调度 | 保留 |
| 存储与数据安全 | 公司 KMS、S3 存储接入、历史文件路径、租户私钥、Redis 事件通信 | 保留 |
| 管理与可观测 | 任务管理、策略管理、Worker/Scheduler 健康、OTel、执行审计 | 保留；执行数据自动清理不再迁移 |
| 部署与运行 | API、Worker、Scheduler、Web 等运行角色及 DevOps 构建、启动和环境配置 | 保留 |

### 2.3 不迁移 / 舍弃功能

本表只记录**功能级**的不再保留项；旧 Adapter、旧 OAuth/S3 实现、Session 调整等属于迁移实现变化，不作为功能舍弃。

| 功能点 | 功能描述 | 舍弃原因 |
| --- | --- | --- |
| 执行数据自动清理 | 按保留周期自动清理托管执行历史数据，用于控制执行数据规模 | 该功能在原基线后续版本中已经被主动删除，当前最终功能状态不包含自动清理；本次升级按最终状态不恢复，避免升级过程中重新引入自动删除历史执行数据的行为 |
