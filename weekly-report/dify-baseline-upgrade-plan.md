...
| 时间 | 阶段 | 主要工作 | 交付 |
| --- | --- | --- | --- |
| 09.21 - 09.25 | 第 1 阶段：基线与平台治理 | 完成 1.17.1 基础运行验证；迁移 SkyOA、超管、默认工作空间与权限 | 新版基础环境 + 账号/工作空间主链路 |
| 09.28 - 10.02 | 第 2 阶段：托管执行底座 | 迁移执行数据模型、Policy、Admission、Scheduler、Worker 与任务管理 | 托管执行主链路 |
| 10.05 - 10.09 | 第 3 阶段：应用执行与调试 | 接入 Workflow、Chatflow、Chat、Completion、Agent 及 Console 调试能力 | 应用执行与调试主链路 |
| 10.12 - 10.16 | 第 4 阶段：存储与数据兼容 | 完成 KMS/S3、历史文件、租户私钥、Human Input 历史数据和监控能力验证 | 存储与历史数据兼容 |
| 10.19 - 10.23 | 第 5 阶段：回归与上线准备 | 数据副本升级、端到端回归、部署切换演练、回滚验证 | 升级验收结果 + 上线准备 |

### 当前进度

| 项目 | 状态 |
| --- | --- |
| Dify 1.17.1 基线确定 | 已完成 |
| 平台构建与部署支持迁入 | 已完成，实际运行待验证 |
| 各功能迁移范围梳理 | 已完成 |
| 功能代码迁移 | 进行中 |
| 数据副本升级验证 | 待开展 |
| 端到端回归与部署切换 | 待开展 |
