...
| 能力方向 | 不迁移 / 舍弃功能 | 功能说明 | 处理 | 原因 |
| --- | --- | --- | --- | --- |
| Console 调试与 Human Input | Console 调试接入托管调度；Human Input 公司托管恢复机制；草稿 Schedule 调试 | 原基线将草稿、单节点、Iteration、Loop、Human Input 暂停恢复和草稿定时调试统一接入公司排队、任务状态、停止和恢复链路 | 本期暂不迁移 | 这些能力都属于调试或暂停恢复链路，1.17.1 已提供完整的官方调试和 Human Input 能力；继续迁移主要增加的是“调试也经过公司托管调度”这一层能力，迁移成本较高但不影响正式应用执行，因此本期优先缩减 |
| 管理与可观测 | 执行数据自动清理 | 按保留周期自动删除托管执行历史数据，控制执行数据长期增长 | 舍弃 | 该功能在原基线后续版本中已经被撤销，当前最终功能状态不再包含自动清理，本次升级不恢复 |
