...
| 能力方向 | 迁移内容 | 不迁移 / 舍弃内容 |
| --- | --- | --- |
| 账号与工作空间 | SkyOA 登录、超级管理员、邀请注册、默认工作空间、工作空间管理与权限继续保留 | 不保留旧版账号、OAuth 和会话的内部实现，直接使用新版已有能力承接 |
| 托管执行引擎 | 保留统一排队、优先级、Scheduler、Standard/Critical Worker、容量控制、任务管理和结果交付 | 不保留旧 Worker 对官方执行底层的替换式实现，不重复实现新版已经具备的执行能力 |
| 应用执行与调试 | Workflow、Chatflow、Chat、Completion、旧 AGENT_CHAT 继续接入托管调度；草稿、单节点、迭代、循环、Human Input、定时触发继续保留 | 不保留旧版各应用执行 Adapter；新版独立 Agent 暂不纳入公司托管调度，先使用官方能力 |
| 存储与数据安全 | 保留公司 KMS、历史文件路径、租户私钥和必要的 S3 适配；继续使用 Redis 事件通信 | 不迁整套旧 S3 客户端，不搬迁历史 S3 对象，不重复实现新版已有的文件上传、下载和预签名能力 |
| 管理与可观测 | 保留任务管理、策略管理、Worker/Scheduler 健康、执行审计以及调度专属监控 | 不重复迁移新版已经具备的通用 Health、Trace 和基础运行监控能力 |
