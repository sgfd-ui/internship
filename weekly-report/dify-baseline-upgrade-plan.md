...
| --- | --- | --- | --- | --- |
| Console 调试 | Workflow、Chatflow 草稿调试接入托管调度 | 将原本直接运行的草稿调试改为创建公司执行任务，经过准入、排队、Scheduler、Worker、状态查询、停止和 SSE 重连 | 本期不迁移 | 1.17.1 已保留官方草稿调试能力。迁移这部分只是让调试任务也经过公司调度中心，不影响正式应用执行，但需要适配大量新版调试接口和前端状态，投入较大 |
| Console 调试 | 单节点、Iteration、Loop 调试接入托管调度 | 为单节点、迭代和循环调试分别增加执行快照、排队状态、结果查询、停止及前端状态同步 | 本期不迁移 | 这些功能与 1.17.1 的节点执行、事件结构和前端调试状态高度耦合，迁移后基本需要按新版调试链路重写；继续使用官方调试即可满足当前功能 |
| Console 调试 | Agent、Completion 调试接入托管调度 | 将 Agent 和文本生成的 Console 调试任务纳入公司任务列表、Worker 和结果交付链路 | 本期不迁移 | 正式 Agent、Completion 执行可以继续迁移，但调试接入属于额外增强。1.17.1 已有官方调试入口，不迁移不会影响正式 API 和 WebApp 执行 |
| Human Input | 公司托管执行的暂停、恢复与重试机制 | 在公司 Job 中增加 generation、暂停状态、表单提交幂等、恢复排队、取消及恢复失败重试 | 本期暂不迁移 | 1.17.1 已有官方 Human Input 能力，而公司的实现与旧执行快照、Job 状态机和 Worker 恢复协议深度绑定。迁移成本较高，先使用官方链路 |
| 定时触发 | 草稿 Schedule 调试接入托管调度 | 让 Console 中的草稿定时任务也创建公司执行任务，并由 Scheduler 和 Worker 执行 | 本期不迁移 | 草稿定时调试不属于正式应用执行的核心需求，并且依赖 Console 调试托管链路。本期只保留正式定时任务是否接入托管调度的迁移，不迁草稿调试增强 |
| 管理与可观测 | 执行数据自动清理 | 按保留期限自动删除公司托管执行的历史任务、快照和相关数据 | 舍弃 | 该功能曾在旧分支中加入，随后又被删除，旧分支最终代码已不再提供这项能力，因此本次升级不恢复 |
| 管理与可观测 | 已撤销的清理命令、配置和运行脚本 | 包括执行数据清理服务、清理命令、环境变量及 `clean-app-execution-data` 脚本 | 舍弃 | 这些内容是执行数据自动清理的配套代码，已经随主功能被旧分支撤销，不应单独迁移 |

这里舍弃的是“公司托管增强层”，不是删除 Dify 原有功能。1.17.1 官方的 Console 调试和 Human Input 继续保留。

需要特别注意：如果正式应用强制全部经过公司托管执行引擎，那么包含 Human Input 的工作流不能直接忽略这部分代码。届时必须二选一：

- 这类工作流继续走 1.17.1 官方执行链路；
- 或把公司 Human Input 暂停恢复机制列回迁移范围。

Standard/Critical、Scheduler、正式执行 Worker、正式应用准入和结果交付不属于舍弃范围。
