...
当前方案的核心是：**先通过规则提取用户请求中的显式参数，再由快速路由锁定明确任务、规划模型拆解复杂请求，两条路径统一进入参数装填；执行层根据任务类型，将任务并行分发给工作流、直接工具或原因分析智能体，最终统一收敛结果并决定展示方式。**
...
| 规划层 | 一个总控智能体同时完成意图识别、参数补全和专业智能体路由 | 快速路由负责明确任务，规划模型负责复杂请求拆解，参数装填作为独立后续步骤 |
...
| 输出 | 返回两组整体指标、日级 CTCVR 趋势、各组头部店铺构成、重叠店铺差异榜以及缺失组、缺失日期、无重叠店铺、噪声门槛等告警。页面不会在一次调用中生成页面榜，需要在相同页面范围下分别运行后比较。EC20 会返回 A/B 不适用的诊断说明，并建议改走召回/策略对比。 | 返回整体比较、选定范围内各实际页面的实验差异、各实验组日级趋势、两套店铺榜、候选与单侧店铺统计以及完整的数据覆盖和 Warning。既可以看逻辑全页面整体，也可以在同一次诊断中看到具体页面差异；EC20 会说明该站点不支持 A/B 实验口径。如果用户实际需要按策略或召回比较效果，系统具备对应能力并由其他 Tool 执行，但不会把原始 A/B 请求自动改写成该分析。 |
...
| EC20 能力边界 | EC20 没有 A/B 维度，A/B 请求不适用；可以转向召回或策略维度继续做效果比较 | EC20 同样不支持 A/B 实验口径，但系统本身支持按策略或召回维度比较效果，由对应 Tool 实现。对于原始 A/B 请求不会自动执行这种替代分析，避免改变用户问题；只有用户明确提出策略/召回比较诉求后才进入对应能力 |
...
当前先整理 Mentor 的原因分析方案作为后续对比基线。当前方案的原因分析职责、输入协议和调查边界后续还需要结合新的设计原则重新调整，因此这里先不提前给出双方差异结论。

#### Mentor 方案

Mentor 的原因分析能力由独立的**推荐精准分析师**负责，专门处理“为什么上涨/下降/异常”“是不是某个店铺导致”“是不是流量结构变化”“是不是配置改了”等因果类问题。它不是在普通效果查询后追加一个固定归因步骤，而是把整个问题作为一次调查任务，在同一 Agent 内完成效果确认、异常识别、贡献拆解、结构检查和配置取证。
...
→ 配置 / 实验变更取证
→ 综合证据判断
→ 输出结论 / 证据 / 排除项 / 不确定性 / 后续建议
```

本质上是一个**按候选原因逐层取证的调查 Agent**，而不是一条只执行固定计算步骤的 Workflow。

##### 输入与默认范围

原因 Agent 接收到的是已经明确或尽量补齐业务范围后的原因问题，不要求用户先把“趋势、显著性、贡献、配置查询”等调查步骤逐项指定。

默认范围包括：

- **站点**：必须最终确定为 EC10 或 EC20；能够从上下文、实验/流量组、召回/策略或店铺反查中确定时直接使用，仍无法确定时才追问。
- **时间**：未指定时默认比较“最近 7 天 vs 前 7 天”；用户明确给出季度、月份、周等窗口时严格按用户范围执行。
- **页面**：未指定时使用整体页面范围；常见页面名称可以自动解析为对应场景。
- **指标**：宽泛的“效果/表现/转化率”会展开到曝光、点击、CTR、CVR、CTCVR、推荐引导 GMV 等相关指标，避免为了常规指标再次追问。

因此它的入口原则是：**用户负责提出要解释的业务现象，Agent 负责补齐不改变问题语义的分析范围，并自行选择后续取证步骤。**

##### 候选原因框架

Mentor 会按照一组固定的候选原因方向逐一寻找证据，主要包括：

1. **数据问题**：数据延迟、缺失日期、覆盖不足或统计口径不一致造成的假异常；
2. **店铺贡献变化**：头部店铺或关键店铺的分子、分母和业务量变化是否推动了整体指标变化；
3. **流量结构变化**：mode、实验/流量组等流量结构是否发生迁移，并进一步影响整体指标；
4. **配置变更**：召回、策略、分层、分桶、权重等配置发布是否与变化时间一致；
5. **实验差异**：实验/对照定义、样本覆盖、分层或过滤条件变化是否造成观察口径或实验效果差异。

这些方向不是最终结论，而是 Agent 的**调查假设集合**。每一个候选原因都需要后续数据证据支持，不能只因为时间上接近就直接判定为原因。

##### 处理逻辑

一次典型原因调查按下面的逻辑逐步进行：

1. **窗口与数据校验**：先检查当前期和基准期是否重叠、倒序，确认缺数、数据滞后、覆盖天数和指标可计算性；缺失值保持为空，不将 null 当作 0。
2. **趋势与变点识别**：查看目标指标趋势，识别变化发生的大致时间，并区分单日尖峰/塌陷与持续性水平抬升或下降。
3. **变化与显著性确认**：先确认用户描述的变化是否真实存在。比例类指标结合比例差异检验、噪声地板和必要的多重检验校正；累计量结合日均、周总或相应窗口差异判断，避免缺数导致总量误判。
4. **店铺贡献拆解**：计算哪些店铺对整体变化贡献最大，同时查看分子变化最大的店铺，并结合样本量和噪声门槛过滤小盘波动。
5. **流量结构拆解**：检查 mode、实验/流量组等结构是否发生变化，并判断这种结构变化对整体指标的影响方向和贡献。
6. **配置与发布对齐**：扫描相关召回、策略、权重、分层、分桶及实验配置的发布时间线，判断配置变更是否发生在指标变化之前或附近，并检查变化方向是否一致。
7. **实验口径检查**：对于实验类问题，继续排查实验/对照定义、桶、分层和过滤条件是否一致，避免把实验口径变化误判成业务效果变化。
8. **综合证据判断**：根据前面各类证据，对候选原因分别判断“证据充分、部分支持或证据不足”，同时明确已经排除的原因和仍无法确认的部分。

这里最重要的边界是：**时间相关性只是证据之一，不等于因果关系。** 配置发布、结构变化或某个店铺变化只有在时间、方向和贡献都能对上时，才会被提升为主要原因；否则只能作为候选或被排除。

##### 证据链

Mentor 最终会围绕几个固定证据层组织原因结论：

| 证据层 | 主要回答的问题 |
| --- | --- |
| 指标趋势与变点 | 变化是否真实发生、从什么时候开始、是瞬时还是持续 |
| 统计证据 | 变化是否超过正常噪声范围，显著性与样本质量是否足够 |
| 店铺贡献 | 哪些店铺真正推动了整体变化，贡献有多大 |
| 流量结构 | mode、实验/流量组等结构变化是否解释整体变化 |
| 配置与发布 | 是否存在时间和方向都能与变化对应的配置变更 |
| 实验口径 | 是否存在实验分层、分桶或覆盖变化造成的假差异 |
| 数据护栏 | 是否存在缺数、数据延迟、口径不支持等会影响结论的问题 |

因此原因结论不是单条工具结果直接生成，而是需要**不同证据层相互印证**。

##### 输出

原因分析最终不是简单返回“原因是 X”，而是组织成以下几个部分：

- **结论**：当前证据最支持的主要原因，以及证据强度；
- **关键证据**：指标变化、变点、贡献对象、流量结构和配置时间线等直接支撑；
- **已排除项**：已经检查但与时间、方向或数据不匹配的候选原因；
- **不确定性**：因为数据缺失、口径限制或证据不足而暂时无法确认的部分；
- **后续建议**：若现有证据不足，说明下一步最值得补充的取证方向。

展示层可以根据结果决定是否附带趋势图和贡献表，但文字结论必须同时说明分析范围和关键护栏；查无数据或不可计算时必须解释原因，不能把 null 改写成 0，也不能把“没有证据”写成“没有问题”。

因此 Mentor 的原因分析可以概括为：

> **先验证现象是否成立，再从数据质量、店铺贡献、流量结构、配置变化和实验口径五类候选原因逐层取证，通过趋势、统计、贡献和时间线形成证据链，最后按证据强弱输出结论、排除项与不确定性。**

当前方案后续需要重点重新审视的，就是**原因 Agent 应接收什么输入、哪些确定性分析由外部 Tool 先完成、哪些调查步骤保留给 Agent 自主决策，以及没有配置数据时原因链应该在哪一步停止**。