...
当前先整理 Mentor 的原因分析方案作为后续对比基线。当前方案的原因分析职责和规划方式还需要结合新的设计原则重新调整，因此本节暂不提前给出双方差异结论。

#### Mentor 方案

Mentor 并不会把“原因分析”理解成在普通效果任务后追加一个固定归因步骤，而是先在规划层判断**用户是不是在问原因**。规划层首先把问题分成效果、配置和原因分析三类；只要问题里包含“为什么”“怎么回事”“是不是配置改了”等因果诉求，整个问题就优先交给原因分析智能体处理，不再继续拆成多个效果分析或配置分析任务。
...
→ 站点解析与默认参数补全
→ 单次调用原因分析智能体
→ 内部完成效果差异、贡献拆解、流量与配置取证
→ 输出结论、证据、排除项与不确定性
```

也就是说，Mentor 采用的是**“规划层只决定是否进入原因调查，具体调查路径交给原因分析智能体内部自主完成”**的方式。

##### 输入

原因分析并不要求用户先把效果查询、贡献拆解、配置查询等步骤逐项说清楚。只要用户给出了一个明确的因果问题，例如：

> CTR 比上月下降多少，哪些店铺拖累最多，为什么？

规划层会把“下降多少、店铺拖累、为什么”视为同一个原因调查目标，整体交给原因分析智能体，而不是先生成周期对比任务、再生成店铺贡献任务、最后再生成原因任务。

进入原因分析前，主要先确定业务范围：

- 站点优先从上下文、实验 ID、召回/策略信息或店铺反查中推导；只有仍无法确定 EC10 / EC20 时才追问。
- 未指定时间时使用系统默认窗口；明确给出季度、月份等时间表达时直接解析。
- 未指定页面时使用整体页面范围；页面名称可以自动映射到对应场景。
- 未指定店铺时默认分析全平台。
- 用户明确要求两站都分析时，EC10 和 EC20 分别生成一个原因分析任务并可并行执行；未明确要求时不会自行两站同时调查。

因此 Mentor 在原因分析入口上的原则是：**尽量补齐分析上下文，只有无法确定站点或用户明确指定的对象无法识别时才追问。**

##### 处理

原因分析智能体内部不采用固定单一算法，而是根据问题动态组织证据。对于“指标为什么下降”这类典型问题，其调查顺序可以概括为：

1. **口径与数据有效性检查**：先确认时间范围、页面和指标口径是否一致，并排查数据缺失、数据延迟等问题，避免把数据问题误判为业务变化。
2. **确认效果变化**：计算目标指标在当前期和基准期之间到底发生了多大变化，先确认“问题是否真实存在”。
3. **贡献拆解**：继续定位哪些店铺、页面、流量结构或其他业务对象对整体变化贡献最大。
4. **结构与配置取证**：结合流量结构、实验差异、推荐配置及其变更记录继续寻找与变化时间、方向相匹配的证据。
5. **综合判断**：综合各类证据形成原因结论，同时保留不能证明的部分和已经排除的候选原因。

这里最重要的特点是：**效果计算、贡献分析、流量检查和配置取证都属于原因分析智能体内部的调查手段，而不是规划层预先拆好的多个独立任务。** 规划层只给出“需要调查什么”，原因分析智能体根据中间结果决定下一步查什么。

##### 输出

Mentor 的原因分析结果不是单纯返回一张指标表，而是组织成完整的调查结论，重点包括：

- **结论**：当前最有证据支持的变化原因是什么；
- **证据**：效果变化、贡献对象、流量变化、配置或实验变化等支撑信息；
- **已排除项**：哪些看似相关的因素经过检查后不能解释当前变化；
- **不确定性**：哪些环节缺少数据或证据，目前只能保留为候选而不能下因果结论。

因此 Mentor 的原因分析能力本质上是一个**证据驱动的 ReAct 调查过程**：Planner 负责识别“这是原因问题”并确定必要业务范围，原因分析智能体则在一次调用内部根据证据逐步选择分析路径，最终返回结论、证据链、排除项和不确定性，而不是把原因分析预先固化成一条固定 Workflow。

当前方案的原因分析部分将在后续结合这一思路重新审视，再补充双方的实现差异。